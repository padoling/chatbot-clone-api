...
.참고
    ...
    ** ${tarot}: 타로 카드 해석 내용
    * variables에 시나리오에서 사용할 변수들을 미리 지정해 놓아야 한다. 단, 사용자 이름은 반드시 'userName'이라는 변수 이름을 사용해야 한다.
...
==== Request URL
    GET /scenario/{scenarioId}?userId={userId}&inputType={inputType}&contentMap={contentMap}&nextMessageNum={nextMessageNum}

==== Request
.[Path Parameters]
include::{snippets}/scenario-get-next-message/path-parameters.adoc[]
.[Request parameters]
include::{snippets}/scenario-get-next-message/request-parameters.adoc[]
.[Request HTTP Example]
include::{snippets}/scenario-get-next-message/http-request.adoc[]
==== Response
.[Response Fields]
include::{snippets}/scenario-get-next-message/response-fields.adoc[]
.[Response HTTP Example]
include::{snippets}/scenario-get-next-message/http-response.adoc[]
...
==== Request
.[Request Parameters]
include::{snippets}/session-get-by-userid/request-parameters.adoc[]
.[Request HTTP Example]
